...
Flit PEMappedTaskExecution::bridgeTxRxSamePE(Packet & p) {
	Flit flit;

	flit.src_id = flit.dst_id = p.src_id;
	flit.timestamp = p.timestamp;
	flit.sequence_no = p.size - 1;
...
#ifdef DEFINE_TM_SIMEXECLOG
		if (p.src_id == p.dst_id) {
			TM_SIMEXECLOG(TM_SIMEXECLOG_PLTOTALLYOUTTOSAMEPE,
				pe->getId(),
				((tm_task *)p.src_task)->getApp()->getId(),
				((tm_task *)p.src_task)->getId(),
				"payload of pb" << p.pblock_id <<
				" is out to the same pe.");
		}
		else {
			TM_SIMEXECLOG(TM_SIMEXECLOG_PLTOTALLYOUTTOWNOC,
				pe->getId(),
				((tm_task *)p.src_task)->getApp()->getId(),
				((tm_task *)p.src_task)->getId(),
				"payload of pb" << p.pblock_id <<
...
void PEMappedTaskExecution::processReceivedFlit(Flit flit, bool samePE) {
	tm_task *t_src, *t_dst;
	tm_pblock_task *t_st;
	...
	if (!samePE) {
		...
		t_st->stat_txdelay[appCurrentMapping].statAddNewValue(ccycle - flit.timestamp);
		t_st->stat_txhops[appCurrentMapping].statAddNewValue(flit.hop_no);
	}
		...
		if (!t_dst->getApp()->alreadyStopped()) {
			...
			for (vector< tm_antecedent_task >::iterator it_at = t_dst->antecedent_tasks.begin(); it_at != t_dst->antecedent_tasks.end(); it_at++) {
				...
				if (t_at->getATask() == t_src && t_at->getPbId() == src_pblock_id) {
					...
					TM_SIMPE2PELATENCYLOG(TM_SIMSTATSLOG_STATLATENCYPE2PE,
						t_dst->getApp()->getId(), t_src->getId(), t_dst->getId(),
						"acum_flits=" << t_st->stat_txdelay[t_dst->getApp()->getCurrentMappingInt()].statGetExecNumber()
						<< ",last=" << t_st->stat_txdelay[appCurrentMapping].statGetLast()
						<< ",min=" << t_st->stat_txdelay[appCurrentMapping].statGetMin()
						<< ",max=" << t_st->stat_txdelay[appCurrentMapping].statGetMax()
						<< ",avg=" << t_st->stat_txdelay[appCurrentMapping].statGetAverage());
